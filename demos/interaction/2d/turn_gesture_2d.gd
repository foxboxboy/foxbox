...
# so hands_2d.gd can be about picking things up.
class_name DemoTurnGesture2D
extends Node


#region Signals

## The gesture moved by [param radians] this frame.
signal turned(radians: float)
...
const DEGREES_PER_PIXEL: float = 0.4
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if _turning:
		turned.emit(deg_to_rad(motion.relative.x * DEGREES_PER_PIXEL))
