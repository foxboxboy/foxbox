...
func _affordability() -> void:
	case("can_be_paid_by")
	var w := FoxSimpleWallet.new()
	w.funds = 100

	var p := FoxSimplePrice.new()
	p.cost = 50
...
func _non_numeric_currency() -> void:
	...
	var w := ScrapWallet.new()
	w.scrap = [&"bolt", &"spring", &"gear"]

	var p := ScrapPrice.new()
