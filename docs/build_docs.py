...
# make_rst.py heads every page it writes with a note naming godotengine/godot, which is right when
# the engine generates its own reference and wrong here. It claims the page came from Godot engine
# sources when it came from ## comments in addons/foxfabric, and the XML source URL it builds
# points into Godot's repo at a path that only exists in this one, so all 64 links 404.
#
# None of it renders, since ".." is a reStructuredText comment. It shows when someone reads the
# committed .rst on GitHub, which is the only place these lines are ever seen.
#
# Rewritten here rather than in the vendored script so its local changes stay at one, which is
# what docs/README.txt documents and what keeps the next Godot upgrade cheap. The Generator line
# keeps the attribution and points at the copy that actually ran.
GODOT_HEADER = re.compile(
    r"\.\. DO NOT EDIT THIS FILE!!!\n"
    r"\.\. Generated automatically from Godot engine sources\.\n"
    r"\.\. Generator: \S+\n"
    r"\.\. XML source: https://github\.com/godotengine/godot/tree/[^/]+/(.+)\.\n"
)


def retarget_header(path):
    text = path.read_text(encoding="utf-8")
    ...
    def replacement(match):
        return (
            ...
            f".. XML source: {match.group(1)}.\n"
        )

    text, count = GODOT_HEADER.subn(replacement, text)
    if count:
        path.write_text(text, encoding="utf-8", newline="\n")
...
def step_collect():
    ...
    for page in sorted(RST_DIR.glob("class_*.rst")):
        ...
        retargeted += retarget_header(WEB_DIR / page.name)
        count += 1
    # index.rst is hand written and uses a glob toctree, so it never needs regenerating
    print("      %d pages copied" % count)

    # A vendored make_rst.py that reworded its header would leave this matching nothing and put
    # Godot's back, one page at a time and without a word.
    if retargeted != count:
        print("      WARNING: header not rewritten on %d page(s), check GODOT_HEADER against "
              "make_rst.py" % (count - retargeted))
