@tool
@icon("uid://dk5df05ts00jb")
class_name FoxHitShapeCast2D
extends ShapeCast2D
## Delivers a [Variant] payload to all [FoxHurtArea2D] nodes it strikes.
##
...
## [member ShapeCast2D.collide_with_areas] belongs to the parent class, so it cannot be given a
...
func _get_configuration_warnings() -> PackedStringArray:
	...
	# Deliberately no warning for a missing shape. ShapeCast2D already reports that itself.
	return warnings
