...
extends Node2D
## A 2D node that drags a [RigidBody2D] to its own position and rotation using forces.
##
## [FoxPhysicsDragger2D] pulls its grabbed body toward itself as it moves. The pull is a force rather than a teleport, so
